...
Please complete the assignments below. 
<ul>
  <li>View QGIS training videos and read Module 2 and 3 of <a href="https://docs.qgis.org/3.10/en/docs/training_manual/">GQIS training manual</a>.</li>
  <li>Answer the questions for this day and review today's code. Answers are in the Answers tab.</li>
  <li>Complete the Feedback form by the end of the day</li>
</ul>
...
```{r,c75, echo=T, eval=F, warning=FALSE, message=FALSE, comment=FALSE}
...
<details open><summary class='drop'>2. Tree diameter distribution figure <b style="color:red">Updated 8-3-20</b></summary>
...
<b style="color:red">Updated Code Chunk 8-3-20</b>
```{r,c80, echo=T}
# Set up labels for facet wrap
cycle_names<-c('Cycle1' = "Cycle 1", 'Cycle2' = 'Cycle 2', 'Cycle3' = 'Cycle 3', 'last4yr' = "Last 4 yrs: 2016 \U2013 2019") #revised 7-30-20
...
<details open><summary class='drop'>2. Tree diameter distribution figure for NCBN park <b style="color:red">Updated 7-30-20</b></summary>
...
tree_dist_plot <- ggplot(data = SAHI_tree_dist_sum, aes(x = size_class, y = avg_dens))+
                         ...
                         geom_errorbar(aes(ymin = avg_dens - se_dens, 
                                       ...
                                       position = position_dodge(0.9))+
                         facet_wrap(~cycle, ncol = 4, labeller = as_labeller(cycle_names))+
                         labs(x = "Tree Diameter Class (cm)", y = "stems/ha")+ #revised 7/20/20
                         theme(axis.text = element_text(size = 6), # change axis label size
                               strip.text = element_text(size = 8), # change facet text size
                               axis.title = element_text(size = 9), # change axis title size
                               axis.text.x = element_text(angle = 45, hjust = 1))+
...
<details open><summary class='drop'>3. Regeneration by size class figure <b style="color:red">Updated 8-3-20</b></summary>
...
Now to save ggplot graph as jpeg. 
...
```{r, c90,echo=T, eval=F, warning=FALSE, message=FALSE, comment=FALSE}
...
<details open><summary class='drop'>5. Seedlings, Saplings, Stocking Index table
<b style="color:red">Updated 7-31-20</b></summary>
...
<b style="color:red">Updated Code Chunk 7-31-20</b>
```{r, c92,echo=T}
RICH_regen_tbl3 <- RICH_regen_tbl2 %>% pivot_wider(names_from = "cycle", 
                                                   values_from = c("seed", "sap", "stock"),
                                                   values_fill = NA) %>% 
                   mutate(plot_num_char = as.character(substr(Plot_Name, 6, nchar(Plot_Name)))) %>%                                              select(Plot_Name, plot_num_char, everything())  
head(RICH_regen_tbl3)
```
...
```{r, c95,echo=T, results='hide', eval=F, warning=FALSE, message=FALSE, comment=FALSE}
...
<details open><summary class='drop'>7. DBI by cycle figure <b style="color:red">Updated 7-30-20</b></summary>
...
RICH_dbi_sum <- RICH_dbi %>% select(Plot_Name, cycle, Deer_Browse_Line_ID) %>% 
                             ...
                             summarize(mean_dbi = mean(Deer_Browse_Line_ID, na.rm = TRUE),
                                       se_dbi = sd(Deer_Browse_Line_ID, na.rm = TRUE)/
                                                sqrt(sum(!is.na(Deer_Browse_Line_ID))),
...
<b style="color:red">Code Chunk Updated 7-30-20</b>
```{r, c97,echo = T}
dbi_plot <- ggplot(data = RICH_dbi_sum, aes(x = cycle, y = mean_dbi))+
                   geom_errorbar(aes(ymin = mean_dbi - se_dbi, 
                                     ...
                                     size = 0.3, #added 7/13
                                     position = position_dodge(0.9))+
                   geom_point(size=2)+ #ordered so points are drawn on top of error bars
                   labs(x = NULL, y = "Average Deer Browse Impact")+
                   theme(axis.text.y = element_text(angle = 45, hjust = 1),
                         axis.text = element_text(size = 8), 
                         axis.title = element_text(size = 9))+
                   scale_x_discrete(labels= c("Cycle 1", "Cycle 2", "Cycle 3", "Last 4 Years"))+
                   scale_y_continuous(limits = c(0.5,5.5), breaks = c(1, 2, 3, 4, 5), #revisd 7/30/20
                                      labels = c("No Impact", "Low", "Moderate", "High", "Severe"))+  
                   theme_FVM()
...
<details open><summary class='drop'>8. Average stocking index by cycle <b style="color:red">Updated 8-3-20</b></summary>
...
stock_plot <- ggplot(data = RICH_stock_final, aes(x = cycle, y = mean_stock))+
                     geom_bar(stat = 'identity', fill = '#8FA7BE', width = 0.8)+
                     geom_errorbar(aes(ymin = mean_stock - se_stock, 
                                       ymax = mean_stock + se_stock, x = cycle), 
                                   color = "#696969", 
                                   width = 0.2, 
                                   size = 0.3)+
                     labs(x = NULL, y = "Average Stocking Index (per sq.m)")+ #revised 7-30-20
                     scale_x_discrete(labels = c("Cycle 1", "Cycle 2", "Cycle 3", "Last 4 Years"))+
                     ...
                     geom_hline(yintercept = 7.958, lty = 3, lwd = 1, color = '#0E5D2C') #Revised 7/30/20
stock_plot
```
...
```{r, c104,echo=T, eval=F, warning=FALSE, message=FALSE, comment=FALSE}
...
### MIDN Metrics 10-15{.tabset}
This tab covers the next 5 metrics related to composition and invasive species. Go ahead and load the packages and import the data. I updated the joinQuadData() function on 7/2/20, so please reinstall the forestMIDN package to make sure you get the same results I do.
```{r, m10_1,echo=T, results='hide', message=FALSE, eval=FALSE}
library(tidyverse)
devtools::install_github("katemmiller/forestMIDN")
library(forestMIDN)
library(rgdal)
...
```
<details open><summary class='drop'>10. Regeneration by composition  <b style="color:red">Updated 7-15-20</b></summary>
For this metric, we're going to use joinRegenData() to pull in the last 4 years of regeneration data for RICH. This time, we'll include all species, including exotic and low canopy. We need to limit the pie charts to 12 or less, because we start to run out of distinct colors and it's harder to distinguish patterns. 
```{r, m10_2, echo=T, results = 'hide'}
RICH_reg_spp <- joinRegenData(park = 'RICH', from = 2016, to = 2019, 
                              speciesType = 'all', canopyForm = 'all')
```

The chunk below is to figure out which species are the most common, and which ones to lump. We're only interested in species that have at least one seedling or sapling count (ie don't want to consider species with only cover), so first we want to filter out species that don't have counts (thanks for catching Jamie!).
```{r, m10_3, echo=T, results = 'hide'}
names(RICH_reg_spp)

RICH_reg_spp2 <- RICH_reg_spp %>% mutate(reg_den = seed.den + sap.den) %>% filter(reg_den > 0)
length(unique(RICH_reg_spp2$Plot_Number)) 
sppdf <- as.data.frame(table(RICH_reg_spp2$Latin_Name)) %>% arrange(desc(Freq))
#View(sppdf) #Ilex opaca for the win!
```

To make life easier, I'm going to make a column of species codes to work with. Otherwise, the spaces between the genus and species require special handling via ``. I also see myself wanting this for several of the metrics, so I'm going to make a function that creates the species codes.
```{r, m10_4, echo=T, results = 'hide'}
...
make_sppcode <- function(df){
  ...
  df2$sppcode[df2$species == 'saccharinum'] <- "ACESAC2" # keeps silver and sugar maple from having duplicate codes
  df2$sppcode[df2$sppcode == 'QUE(RE'] <- "QUERED" # fixes Quercus (red group)
  df2$sppcode[df2$sppcode == 'QUE(WH'] <- "QUEWHI" # fixes Quercus (white group)
  return(df2)
}

RICH_reg_spp2 <- make_sppcode(RICH_reg_spp2) 

```

The first ifelse() statement is filling species column with "spp" if the word function returns NA, which happens when only the genus is recorded. The ifelse function takes 3 arguments. The first argument is something that is either true or false. So, if the code for species returns an NA (eg only genus recorded for Latin Name), return spp, otherwise, return the text after the first space, which is the species. The toupper function makes it all capital.

The paste0 function pastes the first 3 letters of the genus and the first 3 letters of the species together into one string. If I had just used paste(), the default would be to add a space between the two strings. The paste0 function doesn't add anything. The last bit of code checks whether Acer saccharinum is in the species list, which would have the same code as Acer saccharum. If silver maple is in the dataset, its code will be ACESAC2. I think that's the only possible duplicate code we're going to encounter, but if you find more, let the group know. 

The next code chunk assigns a species group to each species. This is where you'll have to look at your own data and make decisions about groupings. We typically lump Fraxinus, Quercus, Carya, Pinus, Prunus (excluding Prunus avium, which is exotic), and Ulmus by genus. Fraxinus, Quercus, Carya, and Pinus are all important species in these parks, and should be a separate category, unless there's <5% of plots containing those species groups. 

If you have more than one Acer, you can lump them too (make sure you only lump native maples- no A. platanoides or A. palmatum). Though, often we only have Acer rubrum. 

Exotic species can be lumped, unless you have a lot of one species, like Ailanthus altissima, and only a few other exotics. In that case, I'd leave Ailanthus on its own. Malus (apple/crabapple) are exotic but not super invasive, so I'd combine that with other exotics. Fagus grandifolia is often a good one to leave on its own. Low canopy species are another group that most or all parks will have. Those can be identified by the Canopy_Exclusion field.

To make these species groups, I'm going to make vectors out of the sppcode column, then use mutate and case_when to make the sppgrp column. Instead of copying the same 3 functions over and over, I made a make_spp_list function to speed things up. 

For every park, you'll want to try the groupings on your own first, then run them by Camilla and Kate, like you did for the vegetation groupings in the maps.

```{r,m10_5, echo=T, results = 'hide'}
make_spp_list <- function(df){
      vec <- df %>% select(sppcode) %>% unique()
      }

lowcan_spp <- RICH_reg_spp2 %>% 
                filter(Canopy_Exclusion == TRUE) %>% 
                filter(Latin_Name != "No species recorded") %>%
                filter(Exotic == FALSE) %>% 
                make_spp_list()


exotic_spp <- RICH_reg_spp2 %>% filter(Exotic == TRUE) %>% make_spp_list()
              
exotic_spp # No exotic tree species in RICH regen! I won't include this in sppgroup then.

carya_spp <- RICH_reg_spp2 %>% filter(genus == "Carya") %>% make_spp_list()

frax_spp <-  RICH_reg_spp2 %>% filter(genus == "Fraxinus") %>% make_spp_list()
               
pinus_spp <- RICH_reg_spp2 %>% filter(genus == "Pinus") %>% make_spp_list()

querc_spp <- RICH_reg_spp2 %>% filter(genus == "Quercus") %>% make_spp_list()

ulmus_spp <- RICH_reg_spp2 %>% filter(genus == "Ulmus") %>% make_spp_list()

```
Next I want to take native species that aren't very common and put them in an other native group. The group_by and numplots is calculating the number of plots a species occurs on, so I can take species that occur in <20% of plots and group them into other native, while leaving more abundant species in their own group. Every park is going to be a little different. You can change the % of plots threshold until you get the number of species groups where you want it. I originally used 30% as the cutoff, but after I removed species that didn't have counts, I ended up with room to add species groups. 

```{r, m10_6, echo=T, results = 'hide'}
grped_spp <- rbind(lowcan_spp, carya_spp, frax_spp, pinus_spp, querc_spp, ulmus_spp)

totnum_plots <- length(unique(RICH_reg_spp2$Plot_Name))

other_nat <- RICH_reg_spp2 %>% group_by(sppcode) %>% 
                               mutate(numplots = sum(!is.na(Latin_Name))/totnum_plots*100) %>% 
                               ungroup() %>% 
                               filter(!(sppcode %in% grped_spp$sppcode)) %>% 
                               filter(Exotic == FALSE) %>% 
                               filter(numplots <= 20) %>% #combining spp. in *CHANGED 7/8* <20% of plots instead of <30
                               make_spp_list()

# I split the filters up to make sure it worked properly, and because they were pulling from
# different tables. 

RICH_reg_spp2 <- RICH_reg_spp2 %>% mutate(sppgrp = case_when(sppcode %in% lowcan_spp$sppcode ~ paste0("LOWCAN"),
                                                             sppcode %in% exotic_spp$sppcode ~ paste0("EXOSPP"),
                                                             sppcode %in% carya_spp$sppcode ~ paste0("CARSPP"),
                                                             sppcode %in% frax_spp$sppcode ~ paste0("FRASPP"),
                                                             sppcode %in% pinus_spp$sppcode ~ paste0("PINSPP"),
                                                             sppcode %in% querc_spp$sppcode ~ paste0("QUESPP"),
                                                             sppcode %in% ulmus_spp$sppcode ~ paste0("ULMSPP"),
                                                             sppcode %in% other_nat$sppcode ~ paste0("OTHNAT"),
                                                             TRUE ~ paste0(sppcode) 
                                                           ),
                                          reg_den = seed.den + sap.den
                                        )
# The TRUE ~ paste0(sppcode) essentially says for any species that doesn't fit in one of the groups above,
# assign the sppcode as its group (eg ACERUB in RICH)

table(RICH_reg_spp2$sppgrp) # made it down to 11 species groups. You may have to do some of this work by hand, 
#and possibly using Excel to get your groupings down to 11 or fewer.

# Now to combine rows of species in the same species group
RICH_reg_spp3 <- RICH_reg_spp2 %>% group_by(Plot_Name, Plot_Number, X_Coord, Y_Coord, sppgrp) %>% 
                                   summarize(reg_den = sum(reg_den, na.rm = TRUE),
                                             .groups = 'keep') #added so dplyer doesn't give warning
table(RICH_reg_spp3$sppgrp)

```

Now that I have the species groupings the way I want them, I can spread on sppgroup and get the final shapefile output. Just in case I dropped plots when I ran filter(reg_den > 0) to get RICH_reg_spp2, I'm going do a left merge with RICH_loc_ev to make sure all the plots are included.

<b style="color:red">Updated Code Chunk (see RICH_loc_ev line) 7-15-20</b>
```{r, m10_7, echo=T, results = 'hide', warning=FALSE, message=FALSE, comment=FALSE}
RICH_reg_wide <- RICH_reg_spp3 %>% 
                    select(Plot_Name, Plot_Number, X_Coord, Y_Coord, sppgrp, reg_den) %>% 
                    arrange(sppgrp) %>% 
                    pivot_wider(names_from = "sppgrp", 
                                values_from = "reg_den", 
                                values_fill = 0) %>% #select(-NOSPE) %>% 
                    ungroup() %>% 
                    select(-Plot_Number, -X_Coord, -Y_Coord) #removing so they merge in with RICH_loc_ev

length(unique(RICH_reg_wide$Plot_Name)) #still 32, so didn't lose any plots in the filter(reg_den > 0) above. 

RICH_loc_ev <- joinLocEvent(park = "RICH", from = 2016, to = 2019) %>% select(Plot_Name, Plot_Number, X_Coord, Y_Coord)

RICH_reg_wide2 <- merge(RICH_loc_ev, RICH_reg_wide, by = "Plot_Name", all.x = TRUE, all.y = TRUE)

```
```{r, m10_8, echo=T, results='hide', eval=F}
coordinates(RICH_reg_wide2) <- ~X_Coord + Y_Coord
proj4string(RICH_reg_wide2) <- CRS("+init=epsg:26918")
writeOGR(RICH_reg_wide2, dsn = './shapefiles', layer = 'RICH_reg_spp', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE) 
```
</details>
</br>

<details open><summary class='drop'>11. Tree composition</summary>
This metric is going to have a similar approach as metric 10, except we'll be using the joinTreeData() function now. I also need to link the tree and plant table data to get Canopy_Exclusion to make lowcanopy group.

```{r m11_1, echo = T, results = 'hide', warning=FALSE, message=FALSE, comment=FALSE}
RICH_tree <- joinTreeData(park = "RICH", from = 2016, to = 2019, 
                          speciesType = 'all', canopyForm = 'all', status = 'live')

RICH_tree2 <- merge(RICH_tree, plants[, c("TSN", "Canopy_Exclusion")], 
                    by = "TSN", all.x = TRUE, all.y = FALSE)

RICH_tree2 <- make_sppcode(RICH_tree2) # add species code to dataset

# Next, make groups (copied make_spp_list function from previous section, in case it's not in your global env.)
make_spp_list <- function(df){
      vec <- df %>% select(sppcode) %>% unique()
      }

lowcan_tr <- RICH_tree2 %>% filter(Canopy_Exclusion == TRUE) %>% 
                            filter(Latin_Name != "No species recorded") %>%
                            filter(Exotic == FALSE) %>% 
                make_spp_list()

exotic_tr <- RICH_tree2 %>% filter(Exotic == TRUE) %>% make_spp_list()
exotic_tr # No exotic live tree species in RICH!

carya_tr <- RICH_tree2 %>% filter(genus == "Carya") %>% make_spp_list()

frax_tr <-  RICH_tree2 %>% filter(genus == "Fraxinus") %>% make_spp_list()
               
pinus_tr <- RICH_tree2 %>% filter(genus == "Pinus") %>% make_spp_list()

querc_tr <- RICH_tree2 %>% filter(genus == "Quercus") %>% make_spp_list()

ulmus_tr <- RICH_tree2 %>% filter(genus == "Ulmus") %>% make_spp_list()

grped_tr <- rbind(lowcan_tr, carya_tr, frax_tr, pinus_tr, querc_tr, ulmus_tr)

totnum_plots <- length(unique(RICH_tree2$Plot_Name)) # to calc. % of plot occurrences

other_nat_tr <- RICH_tree2 %>% group_by(sppcode) %>% 
                               mutate(numplots = sum(!is.na(Latin_Name))/totnum_plots*100) %>% 
                               ungroup() %>% 
                               filter(!(sppcode %in% grped_tr$sppcode)) %>% 
                               filter(Exotic == FALSE) %>%
                               filter(numplots <= 30) %>% #combining spp. in <30% of plots
                               make_spp_list()
other_nat_tr

# Creating the sppgrp variable
RICH_tree2 <- RICH_tree2 %>% mutate(sppgrp = case_when(sppcode %in% lowcan_tr$sppcode ~ paste0("LOWCAN"),
                                                       sppcode %in% exotic_tr$sppcode ~ paste0("EXOSPP"),
                                                       sppcode %in% carya_tr$sppcode ~ paste0("CARSPP"),
                                                       sppcode %in% frax_tr$sppcode ~ paste0("FRASPP"),
                                                       sppcode %in% pinus_tr$sppcode ~ paste0("PINSPP"),
                                                       sppcode %in% querc_tr$sppcode ~ paste0("QUESPP"),
                                                       sppcode %in% ulmus_tr$sppcode ~ paste0("ULMSPP"),
                                                       sppcode %in% other_nat_tr$sppcode ~ paste0("OTHNAT"),
                                                       TRUE ~ paste0(sppcode)
                                                        ))
table(RICH_tree2$sppgrp) # made it down to 12 species groups. You may have to do some of this work by hand, 
# and possibly using Excel to get your groupings down to 12 or fewer.

# Now to combine rows of species in the same species group
RICH_tree3 <- RICH_tree2 %>% group_by(Plot_Name, Plot_Number, X_Coord, Y_Coord, sppgrp) %>% 
                                      summarize(BA_m2ha = sum(BA_cm2, na.rm = TRUE)/400)

table(RICH_tree3$sppgrp) # number of plots a species group occurs in

# Make the dataset wide, so there's a row for each plot and a column for each species group
RICH_tree_wide <- RICH_tree3 %>% 
                    select(Plot_Name, Plot_Number, X_Coord, Y_Coord, sppgrp, BA_m2ha) %>% 
                    arrange(sppgrp) %>% 
                    pivot_wider(names_from = "sppgrp", 
                                values_from = "BA_m2ha", 
                                values_fill = 0) %>% 
  ungroup()#%>% select(-NOSPE)

head(RICH_tree_wide)

# Now save output to shapefile
coordinates(RICH_tree_wide) <- ~X_Coord + Y_Coord
proj4string(RICH_tree_wide) <- CRS("+init=epsg:26918")
writeOGR(RICH_tree_wide, dsn = './shapefiles', layer = 'RICH_tree_spp', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE) 
...
<details open><summary class='drop'>12. Invasive cover by cycle <b style="color:red">Updated 8-12-20</b></summary>
<s>This metric is more straightforward than the previous two.</s> The final dataset will have a row for each plot and a column with average invasive cover for each cycle. But first, because we're looking at trends here, we need to make sure we're only including species that have been on the indicator list since 2007. That way, any trends we see are due to changes in plant abundance, not changes in the indicator list. 

Fortunately, we mostly figured this out for our regional invasive trend analysis, so I'll load that list, named Invasive_List.csv, and use it to only include invasive species in the analysis. I copied that list to the google drive, so please download it and put it in your data folder. In this dataset, species with a 1 in the MIDN column should be included in our analysis. Species with 0 will not be included in this metric, but will be included in metrics that only consider the last 4 years. 

```{r m12_2, echo=T, results = 'hide'}
inv_list <- read.csv("../data/Invasive_List.csv")
head(inv_list) #Catalpa spp are not on list, so don't need to kick out
MIDN_inv_list <- inv_list %>% filter(MIDN_INV == 1 & Year_added ==2007) %>% select(Latin_Name)

#View(MIDN_inv_list)
#Now making an invasive only list to filter against
```

Now time to pull in quadrat data for RICH. First we pull in the quadrat data for exotic species and filter out species that aren't on the MIDN_inv_list. <font style="color:red">Update: We also need to pull in % cover of invasive tree species, which are stored with the quadrat seedling data, instead of the quadrat species data for all years except 2019. We'll need to remove 2019 % cover of tree seedlings in quadrats too, so we're not double counting (Sorry the MIDN protocol is so complicated...).</font> Next we sum up the cover of each invasive at the plot level. We'll have to merge this dataset in with a table that includes all plots and visits for RICH, since plots without invasives will get dropped when we filter out species not on the invasive list. After merging, we'll pivot the data so we have a column for every park and a field for every cycle with invasive cover as the values.

<b style="color:red">Updated Code Chunk 8-12-20</b>
```{r m12_3, echo=T, results = 'hide'}
RICH_exo_spp <- joinQuadData(park = "RICH", from = 2007, to = 2019, speciesType = 'exotic')
length(unique(RICH_exo_spp$Plot_Name)) #making sure it's still 32

# In addition to dropping exotic species that aren't invasive, we also need to remove 
# tree species that were only picked up in 2019 because of a protocol change.
# Fortunately the joinQuadData function returns a field called "new_2019" that we can filter on
RICH_inv_spp <- RICH_exo_spp %>% filter(Latin_Name %in% MIDN_inv_list$Latin_Name) %>% 
                                 filter(new_2019 == FALSE)

length(unique(RICH_inv_spp$Plot_Name)) # Now it's 24, meaning that plots without species on the MIDN_inv_list
# were removed. We'll add those back in with a left_join near the end.

# Now to pull in invasive tree seedlings from the quadrat seedling table using joinRegenData
RICH_exo_seed <- joinRegenData(park = "RICH", from = 2007, to = 2019, speciesType = 'exotic')
RICH_inv_seed <- RICH_exo_seed %>% filter(Latin_Name %in% MIDN_inv_list$Latin_Name)

# We need to make the two datasets have the same column names and order so we can combine them
RICH_inv_spp2 <- RICH_inv_spp %>% select(Event_ID, Unit_Code, Plot_Name, Plot_Number, 
                                         Year, cycle, TSN, Latin_Name, avg.cover)

RICH_inv_seed2 <- RICH_inv_seed %>% select(names(RICH_inv_spp2))

# Now combine the datasets
RICH_inv_comb <- rbind(RICH_inv_spp2, RICH_inv_seed2)

RICH_inv_comb2 <- RICH_inv_comb %>% group_by(Event_ID, Plot_Name, cycle) %>% 
                                    summarize(avg.cover = sum(avg.cover, na.rm = TRUE), 
                                              .groups = "keep") # new thing dplyr wants

length(unique(RICH_inv_comb2$Plot_Name)) #24 plots- need to add plots without invasives

RICH_plot_ev <- joinLocEvent(park = "RICH", from = 2007, to = 2019) # left data set to merge to
length(unique(RICH_plot_ev$Plot_Name)) #32

RICH_inv_merge <- merge(RICH_plot_ev[ , c("Event_ID", "Plot_Name", "Plot_Number", "cycle", "X_Coord", "Y_Coord")],
                        RICH_inv_comb2, by = c("Event_ID", "Plot_Name", "cycle"), all.x = TRUE, all.y = TRUE)

table(RICH_inv_merge$Plot_Name, RICH_inv_merge$cycle) # Every plot has a 1 for first 3 cycles, and only
# panel 4 has first panel of plots, so that looks good.

# Now to change NA to 0 for plots that don't have any invasives for a given visit
table(complete.cases(RICH_inv_merge$avg.cover)) # a bunch of NAs in the data
RICH_inv_merge$avg.cover[is.na(RICH_inv_merge$avg.cover)] <- 0
table(complete.cases(RICH_inv_merge$avg.cover)) # no there aren't any NAs
```

Now time to pivot the table wide by cycle, and then save to shapefile.
```{r m12_4, echo=T, results = 'hide', warning=FALSE, message=FALSE, comment=FALSE}

RICH_inv_wide <- RICH_inv_merge %>% select(Plot_Name, cycle, Plot_Number, X_Coord, Y_Coord, avg.cover) %>% 
                                    arrange(Plot_Name, cycle) %>% 
                                    pivot_wider(names_from = cycle, values_from = avg.cover) 
# We won't values fill this, because there are plots in cycle 4 that haven't been sampled. We want those to be NA.

# Now to save output as shapefile
coordinates(RICH_inv_wide) <- ~X_Coord + Y_Coord
proj4string(RICH_inv_wide) <- CRS("+init=epsg:26918")
writeOGR(RICH_inv_wide, dsn = './shapefiles', layer = 'RICH_inv_cycle', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE) 
...
<details open><summary class='drop'>13. Invasive cover table <b style="color:red">Updated 8-12-20</b></summary>
The shapefile we generated in metric 12 has the same information that we'll provide in this metric, but as a table. To save time, I'll just take the data from the shapefile and save it as a csv. When you create spatial dataframes in R, the data (eg think if this as the attribute table in GIS) can be accessed via the data slot, which you use \@data to access. If you're starting from scratch, just run the code for metric 12 up until you create RICH_inv_wide. Before converting to shapefile, save the data frame as a csv.  

<font style="color:red">Note: The easiest way to pad the plot numbers with 0s in Excel is to use this formula: =right(<Plot_Name column>, 3). That should pull the right 3 digits from the Plot_Name column. To be safe, you'll want to copy and "Paste Special", and then paste as Values. Then right click on the Plot Number column, format as text. Finally save as .xlsx instead of .csv. </font>

<b style="color:red">Be sure to rerun this code chunk, so Metric 13 correctly incorporates invasive tree % cover.</b>
```{r m13_1, results = 'hide', echo=TRUE}
RICH_inv_tbl <- RICH_inv_wide@data #The plot number is padded here. 
write.csv(RICH_inv_tbl, "./tables/Invasive_cover_table_M13.csv", row.names = FALSE)
```

<b style="color:red">For those of you separating this table by unit, I coded that below starting from the beginning of Metric 12.</b>
```{r m13_1b, results = 'hide', echo=TRUE}
FRSP_exo_spp <- joinQuadData(park = "FRSP", from = 2007, to = 2019, speciesType = 'exotic')
length(unique(FRSP_exo_spp$Plot_Name)) #making sure it's still 104

# In addition to dropping exotic species that aren't invasive, we also need to remove 
# tree species that were only picked up in 2019 because of a protocol change.
# Fortunately the joinQuadData function returns a field called "new_2019" that we can filter on
FRSP_inv_spp <- FRSP_exo_spp %>% filter(Latin_Name %in% MIDN_inv_list$Latin_Name) %>% 
                                 filter(new_2019 == FALSE)

length(unique(FRSP_inv_spp$Plot_Name)) # Now it's 27, meaning that plots without species on the MIDN_inv_list
# were removed. We'll add those back in with a left_join near the end.

# Now to pull in invasive tree seedlings from the quadrat seedling table using joinRegenData
FRSP_exo_seed <- joinRegenData(park = "FRSP", from = 2007, to = 2019, speciesType = 'exotic')
FRSP_inv_seed <- FRSP_exo_seed %>% filter(Latin_Name %in% MIDN_inv_list$Latin_Name)

# We need to make the two datasets have the same column names and order so we can combine them
FRSP_inv_spp2 <- FRSP_inv_spp %>% select(Event_ID, Unit_Code, Plot_Name, Plot_Number, 
                                         Year, cycle, TSN, Latin_Name, avg.cover)

FRSP_inv_seed2 <- FRSP_inv_seed %>% select(names(FRSP_inv_spp2))

# Now combine the datasets
FRSP_inv_comb <- rbind(FRSP_inv_spp2, FRSP_inv_seed2)

FRSP_inv_comb2 <- FRSP_inv_comb %>% group_by(Event_ID, Plot_Name, cycle) %>% 
                                    summarize(avg.cover = sum(avg.cover, na.rm = TRUE), 
                                              .groups = "keep") # new thing dplyr wants

length(unique(FRSP_inv_comb2$Plot_Name)) #28 plots- need to add plots without invasives

FRSP_loc_ev_subunit <- joinLocEvent(park = "FRSP", from = 2007, to = 2019, output = "verbose") %>% 
                       mutate(subunit = substr(Unit_ID, 6, nchar(Unit_ID))) %>% 
                       select(Event_ID, Plot_Name, Plot_Number, cycle, 
                              X_Coord, Y_Coord, subunit)

length(unique(FRSP_loc_ev_subunit$Plot_Name)) #104

FRSP_inv_merge <- merge(FRSP_loc_ev_subunit,
                        FRSP_inv_comb2, by = c("Event_ID", "Plot_Name", "cycle"), all.x = TRUE, all.y = TRUE)

table(FRSP_inv_merge$Plot_Name, FRSP_inv_merge$cycle) # Every plot has a 1 for first 3 cycles, and only
# panel 4 has first panel of plots, so that looks good.

# Now to change NA to 0 for plots that don't have any invasives for a given visit
table(complete.cases(FRSP_inv_merge$avg.cover)) # a bunch of NAs in the data
FRSP_inv_merge$avg.cover[is.na(FRSP_inv_merge$avg.cover)] <- 0
table(complete.cases(FRSP_inv_merge$avg.cover)) # no there aren't any NAs

FRSP_inv_wide <- FRSP_inv_merge %>% select(Plot_Name, cycle, Plot_Number, subunit, avg.cover) %>% 
                                    arrange(Plot_Name, cycle) %>% 
                                    pivot_wider(names_from = cycle, values_from = avg.cover) %>% 
                                    arrange(subunit, Plot_Name)
# We won't values fill this, because there are plots in cycle 4 that haven't been sampled. We want those to be NA.
#View(FRSP_inv_wide)
length(unique(FRSP_inv_wide$Plot_Name)) #104

write.csv(FRSP_inv_wide, "./tables/Invasive_cover_table_M13.csv", row.names = FALSE)
```
...
<details open><summary class='drop'>14. Invasive cover by species <b style="color:red">Updated 8-12-20</b></summary>
The approach for this metric is similar for metric 10 and 11. You'll need to determine the most common invasive species, which will have their own pie color. If you have >12 species, then you'll need to lump the less common species. The final dataset will have a row for every plot, and a column for each species. The values will be the average cover of that species. We're only using the last 4 years of data for this metric, so we can include all invasive species, instead of only the ones that have been on the indicator list since 2007.

I'm going to use GETT, since there are a lot of invasive species in that park.
<br>
<b style="color:red">Updated Code Chunk 8-12-20</b>
```{r m14_1, results = 'hide', echo = TRUE}
make_inv_sppcode <- function(df){
  df2 <- df %>% mutate(genus = word(Latin_Name, 1),
                       species = ifelse(is.na(word(Latin_Name, 2)), "spp",
                                       word(Latin_Name,2)),
                       sppcode = toupper(paste0(substr(genus, 1, 3),
  ...
  df2$sppcode[df2$sppcode == 'NASPP'] <- "NOSPP"
  df2$sppcode[df2$sppcode == 'NOSPE'] <- "NOSPP"
  df2$sppcode[df2$sppcode == "LON-"]<-"LONSPP"
  return(df2)
}

GETT_inv_spp <- joinQuadData(park = "GETT", from = 2016, to = 2019, speciesType = 'exotic') %>% 
                  filter(new_2019 == FALSE)

GETT_inv_seed <- joinRegenData(park = "GETT", from = 2016, to = 2019, speciesType = 'exotic')

GETT_inv_spp2 <- GETT_inv_spp %>% select(Event_ID, Unit_Code, Plot_Name, Plot_Number, X_Coord, Y_Coord,
                                         Year, cycle, TSN, Latin_Name, avg.cover)

GETT_inv_seed2 <- GETT_inv_seed %>% select(names(GETT_inv_spp2))

GETT_inv_pie <- rbind(GETT_inv_spp2, GETT_inv_seed2)

GETT_inv_pie <- make_inv_sppcode(GETT_inv_pie) 

top_spp <- data.frame(table(GETT_inv_pie$sppcode)) %>% arrange(-Freq)

sort(unique(GETT_inv_pie$sppcode))
```
There are 16 species on the list in GETT, requiring us to lump like with Metrics 11 & 12. <br>

Note that the groups below include more species than occur in GETT to help with groupings for other parks. The exo_herb group, for example, should be used for all parks, and will be labeled as other exotic herbs in the map.<br>
<b style="color:red">Updated Code Chunk 8-12-20</b>
```{r m14_2, results = 'hide', echo=TRUE}
exo_herb <- data.frame(sppcode = c("DUCIND", "CIRARV", "GLEHED", "PERLON", "VINMIN")) #added 7/22
inv_herb <- data.frame(sppcode = c("ALLPET", "CARIMP", "RANFIC")) #revised 7/22
inv_woody <- data.frame(sppcode = c("DIOOPP", "CELORB", "EUOALA", "ELAANG", "ELASPP", "ELAUMB", "LESCUN", "LONSPP", "VIBSIE"))

GETT_inv_pie <- GETT_inv_pie %>% mutate(sppgrp = case_when(sppcode %in% exo_herb$sppcode ~ paste0("EXOHERB"),#added 7/22
                                                           sppcode %in% inv_herb$sppcode ~ paste0("INVHERB"),
                                                           sppcode %in% inv_woody$sppcode ~ paste0("INVWDY"),
                                                           TRUE ~ paste0(sppcode)
  
  
))

# Summarize so only 1 sppgrp per plot
GETT_inv_pie2 <- GETT_inv_pie %>% group_by(Unit_Code, Plot_Name, Plot_Number, X_Coord, Y_Coord, sppgrp) %>% 
                                  summarize(avg.cover = sum(avg.cover, na.rm=T),
                                            .groups = 'keep') %>% arrange(sppgrp) %>% 
                                  ungroup() # added 7/15
#View(GETT_inv_pie2)
table(GETT_inv_pie2$sppgrp)
```

Now we need to take the sppcode and spread it wide and make into a shapefile.<br>
<b style="color:red">Updated code chunk 8-12-20</b>
```{r m14_3, results = 'hide', echo = TRUE, warning=FALSE, message = FALSE, comment = FALSE}

GETT_inv_pwide <- GETT_inv_pie2 %>% select(Plot_Name, Plot_Number, X_Coord, Y_Coord, sppgrp, avg.cover) %>% 
                                    pivot_wider(names_from = sppgrp,
                                               values_from = avg.cover, 
                                               values_fill = 0) 

# Every plot has an invasive in GETT, but other parks may have NOSPP listed as a column. We'll want
# to remove those columns before saving to shapefile.
GETT_inv_pwide <- GETT_inv_pwide[ , -which(names(GETT_inv_pwide) %in% c("NOSPP"))]

length(unique(GETT_inv_pwide$Plot_Name)) #33- the correct number. I checked and the code should also return plots without invasives. There just aren't any of those in GETT, unfortunately.

coordinates(GETT_inv_pwide) <- ~X_Coord + Y_Coord
proj4string(GETT_inv_pwide) <- CRS("+init=epsg:26918")
writeOGR(GETT_inv_pwide, dsn = './shapefiles', layer = 'GETT_inv_spp', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE) 
...
<details open><summary class='drop'>15. Invasive species plot frequency <b style="color:red">Updated 8-19-20</b></summary>
This table is going to use the makeSppList() function to determine the number of plots per cycle a species has been found in. The complicating factor here is that some species have been added to the indicator list over time, and we need to indicate that in the table by adding a new column for year added. The Invasive_list.csv has that information, which we'll join to the dataset to include a Year_added field.

```{r m15_1, results = 'hide', echo = TRUE, warning=FALSE, message = FALSE, comment = FALSE}
# Load invasive list, if not already in global environment from previous metrics
inv_list <- read.csv("../data/Invasive_List.csv") %>% select(Latin_Name, Year_added)
names(inv_list)

if(length(inv_list[(inv_list$Latin_Name == "Duchesnia indica"),]) == 0){
  inv_list %>% add_row(., Latin_Name = "Duchesnia indica", Year_added = 2007)}

if(length(inv_list[(inv_list$Latin_Name == "Cirsium arvense"),]) == 0){
  inv_list %>% add_row(., Latin_Name = "Cirsium arvense", Year_added = 2007)}

# Create species list of exotics found in RICH in all years
RICH_exo_spplist <- makeSppList(park = "RICH", from = 2007, to = 2019, speciesType = "exotic")

# Merge species list of occurrences with MIDN invasive list
RICH_inv_spplist2 <- merge(inv_list, 
                           RICH_exo_spplist[ , c("Plot_Number", "Latin_Name", "Common", "present", "cycle")], 
                           by = "Latin_Name", 
                           all.x = FALSE, all.y = TRUE)

# Summarize by species
RICH_inv_spplist3 <- RICH_inv_spplist2 %>% filter(!is.na(Year_added)) %>% 
...
RICH_inv_spplist3$Common <- gsub(",.*$", "\\1", RICH_inv_spplist3$Common) # The first two arguments are java
# code to split the string using , and then take the first string.

sort(unique(RICH_inv_spplist3$Common)) # I prefer calling Persicaria perfoliata mile-a-minute
RICH_inv_spplist3$Common[RICH_inv_spplist3$Common == "Asiatic tearthumb"] <- "mile-a-minute"
# I also prefer calling Vinca minor lesser periwinkle
RICH_inv_spplist3$Common[RICH_inv_spplist3$Common == "myrtle"] <- "lesser periwinkle"
sort(unique(RICH_inv_spplist3$Common)) # I prefer calling Persicaria perfoliata mile-a-minute
# That looks better!

# Pivot wide so there's a column for each cycle and a 
RICH_invlist_wide <- RICH_inv_spplist3 %>% pivot_wider(names_from = cycle, 
                                                       values_from = numplots, 
                                                       values_fill = 0) 
```
Note that we filled NAs with 0 in the step above because we summarized at the species level for each cycle and 0 means we didn't find it in that cycle. We next want to add the total number of of plots sampled each cycle, so it's clear that cycle 4 is only partial. We'll do that by summarizing data from joinLocEvent() and then merging to RICH_invlist_wide. We'll also merge this dataframe with plants to incorporate the common name for each species.

```{r m15_2, echo=T, results = 'hide', warning=FALSE, message = FALSE, comment = FALSE}
RICH_plot_events <- joinLocEvent(park = "RICH", from = 2007, to = 2019)
RICH_plot_nums <- RICH_plot_events %>% group_by(cycle) %>% 
                                       summarize(numvisits = sum(!is.na(Plot_Name))) %>% 
                                       pivot_wider(names_from = cycle, values_from = numvisits) %>% 
                                       ungroup()
```
The easiest way to bind the plot number and invasive list together is to make the RICH_plot_nums have the same column names as RICH_invlist2, then to bind the rows (rbind) together.

```{r m15_3, echo = TRUE, results = 'hide', warning=FALSE, message = FALSE, comment = FALSE}
names(RICH_invlist_wide)
RICH_plot_nums$Latin_Name <- as.character("Number of plots")
RICH_plot_nums$Common <- as.character(NA)
RICH_plot_nums$Year_added <- as.numeric(NA)

RICH_plot_nums <- RICH_plot_nums[ , names(RICH_invlist_wide)] #using names from RICH_invlist2 to order columns the same

RICH_invtbl <- rbind(RICH_invlist_wide, RICH_plot_nums)
```
For the species that are exotic but not invasive, you'll want to put a * next to the latin name in the table, and add in the caption: "Species that are exotic to the mid-Atlantic region, but not considered invasive are indicated by *." The species that should get this star are Duchesnia indica, Glechoma hederacea, Persicaria longiseta, and Vinca minor. I'm going to do this in the code, but you can also do this by hand.
</br>

<b style="color:red">Updated code chunk 8-19-20</b>
```{r m_15_4, echo=TRUE, results = 'hide', warning=FALSE, message = FALSE, comment = FALSE}
exospp_list <- c("Duchesnia indica", "Cirsium arvense", "Glechoma hederacea", 
                 "Persicaria longiseta", "Prunus avium", "Vinca minor")

RICH_invtbl <- RICH_invtbl %>% mutate(Latin_Name = ifelse(Latin_Name %in% exospp_list, paste0(Latin_Name, "*"),
                                                          paste0(Latin_Name)))

write.csv(RICH_invtbl, "./tables/RICH_invspp_plot_freq_M15.csv", row.names = FALSE)
```
