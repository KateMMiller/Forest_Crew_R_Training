...
if(length(inv_list[(inv_list$Latin_Name == "Duchesnea indica"),]) == 0){
  inv_list %>% add_row(., Latin_Name = "Duchesnea indica", Year_added = 2007)}
...
```
For the species that are exotic but not invasive, you'll want to put a * next to the latin name in the table, and add in the caption: "Species that are exotic to the mid-Atlantic region, but not considered invasive are indicated by *." The species that should get this star are Duchesnea indica, Glechoma hederacea, Persicaria longiseta, and Vinca minor. I'm going to do this in the code, but you can also do this by hand.
</br>

<b style="color:red">Updated code chunk 8-19-20</b>
```{r m_15_4, echo=TRUE, results = 'hide', warning=FALSE, message = FALSE, comment = FALSE}
exospp_list <- c("Duchesnea indica", "Cirsium arvense", "Glechoma hederacea", 
