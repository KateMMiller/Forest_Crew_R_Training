...
To make life easier, I'm going to make a column of species codes to work with. Otherwise, the spaces between the genus and species require special handling via ``.
```{r, m10_4, echo=T, results = 'hide'}
RICH_reg_spp <- RICH_reg_spp %>% 
...
RICH_reg_spp$sppcode[RICH_reg_spp$species == 'saccharinum'] <- "ACESAC2"
RICH_reg_spp$sppcode[RICH_reg_spp$sppcode == 'QUE(RE'] <- "QUERED" #fixes Quercus (red group)
RICH_reg_spp$sppcode[RICH_reg_spp$sppcode == 'QUE(WH'] <- "QUEWHI" # fixes Quercus (white group)
...
To make these species groups, I'm going to make vectors out of the sppcode column, then use mutate and case_when to make the sppgrp column. Instead of copying the same 3 functions over and over, I made a make_spp_vector function to speed things up. 
...
exotic_spp <- RICH_reg_spp %>% filter(Exotic == TRUE) %>% make_spp_vector()
              
exotic_spp # No exotic tree species in RICH regen! I won't include this in sppgroup then.

carya_spp <- RICH_reg_spp %>% filter(genus == "Carya") %>% make_spp_vector()

frax_spp <-  RICH_reg_spp %>% filter(genus == "Fraxinus") %>% make_spp_vector()
               
pinus_spp <- RICH_reg_spp %>% filter(genus == "Pinus") %>% make_spp_vector()

querc_spp <- RICH_reg_spp %>% filter(genus == "Quercus") %>% make_spp_vector()

ulmus_spp <- RICH_reg_spp %>% filter(genus == "Ulmus") %>% make_spp_vector()
...
table(RICH_reg_spp$sppgrp) # made it down to 12 species groups. You may have to do some of this work by hand, and possibly using Excel to get your groupings down to 12 or fewer.
...
RICH_tree2 <- RICH_tree2 %>% mutate(genus = word(Latin_Name, 1),
                                   species = ifelse(is.na(word(Latin_Name, 2)), "spp", 
                                                    word(Latin_Name, 2)),
                                   sppcode = toupper(paste0(substr(genus, 1, 3),
                                                     substr(species, 1, 3))))

RICH_tree2$sppcode[RICH_tree2$species == 'saccharinum'] <- "ACESAC2"
RICH_tree2$sppcode[RICH_tree2$sppcode == 'QUE(RE'] <- "QUERED" #fixes Quercus (red group)
RICH_tree2$sppcode[RICH_tree2$sppcode == 'QUE(WH'] <- "QUEWHI" # fixes Quercus (white group)

# Next, make groups (copied make_spp_vector function from previous section, in case it's not in your global env.)
make_spp_vector <- function(df){
      vec <- df %>% select(sppcode) %>% unique()
      }
...
exotic_tr <- RICH_tree2 %>% filter(Exotic == TRUE) %>% make_spp_vector()
exotic_tr # No exotic live tree species in RICH!

carya_tr <- RICH_tree2 %>% filter(genus == "Carya") %>% make_spp_vector()

frax_tr <-  RICH_tree2 %>% filter(genus == "Fraxinus") %>% make_spp_vector()
               
pinus_tr <- RICH_tree2 %>% filter(genus == "Pinus") %>% make_spp_vector()

querc_tr <- RICH_tree2 %>% filter(genus == "Quercus") %>% make_spp_vector()

ulmus_tr <- RICH_tree2 %>% filter(genus == "Ulmus") %>% make_spp_vector()
...
table(RICH_tree2$sppgrp) # made it down to 12 species groups. You may have to do some of this work by hand, and possibly using Excel to get your groupings down to 12 or fewer.
...
MIDN_inv <- inv_list %>% filter(MIDN == 1) 
#Now making an invasive only list to filter against
MIDN_inv_list <- MIDN_inv %>% filter(MIDN == 1) %>% select(Latin_Name)
MIDN_inv_list
```
...
length(unique(RICH_inv_spp$Plot_Name)) # Now it's 24, meaning that plots without invasives were removed. 
# We'll add those back in with a left_join near the end.
head(RICH_inv_spp)
