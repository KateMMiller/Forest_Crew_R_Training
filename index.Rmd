...
This tab covers the next 5 metrics related to composition and invasive species. Go ahead and load the packages and import the data.
```{r, c72b,echo=T, results='hide', message=FALSE}
library(tidyverse)
library(forestMIDN)
library(rgdal)
...
```
<details open><summary class='drop'>10. Regeneration by composition map</summary>
...
```{r, nc4, echo=T, results = 'hide'}
make_spp_vector <- function(df){
      vec <- df %>% select(sppcode) %>% unique()
      vec2 <- as.character(dput(vec$sppcode))
      }
...
```{r, nc5, echo=T, results = 'hide'}
grped_spp <- c(lowcan_spp, carya_spp, frax_spp, pinus_spp, querc_spp, ulmus_spp)
...
other_nat <- RICH_reg_spp %>% group_by(sppcode) %>% 
                              ...
                              ungroup() %>% 
                              filter(!sppcode %in% grped_spp & Exotic == FALSE) %>%
                              filter(numplots <= 30) %>% #combining spp. in <30% of plots
                              make_spp_vector()


RICH_reg_spp <- RICH_reg_spp %>% mutate(sppgrp = case_when(sppcode %in% lowcan_spp ~ paste0("LOWCAN"),
                                                           sppcode %in% exotic_spp ~ paste0("EXOSPP"),
                                                           sppcode %in% carya_spp ~ paste0("CARSPP"),
                                                           sppcode %in% frax_spp ~ paste0("FRASPP"),
                                                           sppcode %in% pinus_spp ~ paste0("PINSPP"),
                                                           sppcode %in% querc_spp ~ paste0("QUESPP"),
                                                           sppcode %in% ulmus_spp ~ paste0("ULMSPP"),
                                                           sppcode %in% other_nat ~ paste0("OTHNAT"),
                                                           TRUE ~ paste0(sppcode)
                                                           ),
...
```{r, nc6, echo=T, results = 'hide', messages='hide', warning = FALSE}
...
<details open><summary class='drop'>11. Tree composition map</summary>
...
```{r nc7, echo = T, results = 'hide', message = 'hide', warning = FALSE}
...
grped_tr <- c(lowcan_tr, carya_tr, frax_tr, pinus_tr, querc_tr, ulmus_tr)
...
other_nat_tr <- RICH_tree2 %>% group_by(sppcode) %>% 
                               mutate(numplots = sum(!is.na(Latin_Name))/totnum_plots*100) %>% 
                               ungroup() %>% 
                               filter(!sppcode %in% grped_tr & Exotic == FALSE) %>%
                               filter(numplots <= 30) %>% #combining spp. in <30% of plots
                               make_spp_vector()

# Creating the sppgrp variable
RICH_tree2 <- RICH_tree2 %>% mutate(sppgrp = case_when(sppcode %in% lowcan_tr ~ paste0("LOWCAN"),
                                                       sppcode %in% exotic_tr ~ paste0("EXOSPP"),
                                                       sppcode %in% carya_tr ~ paste0("CARSPP"),
                                                       sppcode %in% frax_tr ~ paste0("FRASPP"),
                                                       sppcode %in% pinus_tr ~ paste0("PINSPP"),
                                                       sppcode %in% querc_tr ~ paste0("QUESPP"),
                                                       sppcode %in% ulmus_tr ~ paste0("ULMSPP"),
                                                       sppcode %in% other_nat_tr ~ paste0("OTHNAT"),
                                                       TRUE ~ paste0(sppcode)
                                                        ))
