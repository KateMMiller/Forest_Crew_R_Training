...
```
<details open><summary class='drop'>10. Regeneration by composition</summary>
...
The chunk below is to figure out which species are the most common, and which ones to lump.
```{r, m10_3, echo=T, results = 'hide'}
sppdf <- as.data.frame(table(RICH_reg_spp$Latin_Name)) %>% arrange(desc(Freq))
...
RICH_reg_spp <- make_sppcode(RICH_reg_spp) 
...
exotic_spp <- RICH_reg_spp %>% filter(Exotic == TRUE) %>% make_spp_list()
              
exotic_spp # No exotic tree species in RICH regen! I won't include this in sppgroup then.

carya_spp <- RICH_reg_spp %>% filter(genus == "Carya") %>% make_spp_list()

frax_spp <-  RICH_reg_spp %>% filter(genus == "Fraxinus") %>% make_spp_list()
               
pinus_spp <- RICH_reg_spp %>% filter(genus == "Pinus") %>% make_spp_list()

querc_spp <- RICH_reg_spp %>% filter(genus == "Quercus") %>% make_spp_list()

ulmus_spp <- RICH_reg_spp %>% filter(genus == "Ulmus") %>% make_spp_list()

```
Next I want to take native species that aren't very common and put them in an other native group. The group_by and numplots is calculating the number of plots a species occurs on, so I can take species that occur in <10% of plots and group them into other native, while leaving more abundant species in their own group. Every park is going to be a little different. 

```{r, m10_6, echo=T, results = 'hide'}
grped_spp <- rbind(lowcan_spp, carya_spp, frax_spp, pinus_spp, querc_spp, ulmus_spp)

totnum_plots <- length(unique(RICH_reg_spp$Plot_Name))

other_nat <- RICH_reg_spp %>% group_by(sppcode) %>% 
...
RICH_reg_spp <- RICH_reg_spp %>% mutate(sppgrp = case_when(sppcode %in% lowcan_spp$sppcode ~ paste0("LOWCAN"),
...
table(RICH_reg_spp$sppgrp) # made it down to 12 species groups. You may have to do some of this work by hand, 
...
RICH_reg_spp2 <- RICH_reg_spp %>% group_by(Plot_Name, Plot_Number, X_Coord, Y_Coord, sppgrp) %>% 
                                  summarize(reg_den = sum(reg_den, na.rm = TRUE))
```

Now that I have the species groupings the way I want them, I can spread on sppgroup and get the final shapefile output.

```{r, m10_7, echo=T, results = 'hide', warning=FALSE, message=FALSE, comment=FALSE}
RICH_reg_wide <- RICH_reg_spp2 %>% 
...
coordinates(RICH_reg_wide) <- ~X_Coord + Y_Coord
proj4string(RICH_reg_wide) <- CRS("+init=epsg:26918")
writeOGR(RICH_reg_wide, dsn = './shapefiles', layer = 'RICH_reg_spp', 
