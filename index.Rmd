...
<details open><summary class='drop'>14. Invasive cover by species</summary>
...
RICH_inv_pie <- joinQuadData(park = "VAFO", from = 2016, to = 2019, speciesType = 'exotic')

RICH_inv_pie <- make_sppcode(RICH_inv_pie) #adding sppcode- function found in Metric 10.

top_spp <- data.frame(table(RICH_inv_pie$sppcode)) %>% arrange(-Freq)
top_spp
```
There are only 10 species on the list, so we don't have to lump. If you needed to lump, you can use similar code that we did for the regen and tree pies, where you identify the species codes you want to lump, and then assign them to an other_inv group. If you have trouble figuring this out, let me know and I'll add an example.

Now we need to take the sppcode and spread it wide and make into a shapefile.
```{r m14_2, results = 'hide', echo = TRUE, warning=FALSE, message = FALSE, comment = FALSE}

RICH_inv_pwide <- RICH_inv_pie %>% select(Plot_Name, Plot_Number, X_Coord, Y_Coord, sppcode, avg.cover) %>% 
                                   pivot_wider(names_from = sppcode, 
                                               values_from = avg.cover, 
                                               values_fill = 0) %>% 
                                   select(-NOSPE) # Remove column created by plots with no invasives recorded. 

coordinates(RICH_inv_pwide) <- ~X_Coord + Y_Coord
proj4string(RICH_inv_pwide) <- CRS("+init=epsg:26918")
writeOGR(RICH_inv_pwide, dsn = './shapefiles', layer = 'RICH_inv_spp', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE) 
...
<details open><summary class='drop'>15. Invasive species plot frequency</summary>
...
RICH_inv_spplist2 <- merge(MIDN_inv_list, 
                           RICH_exo_spplist[ , c("Plot_Number", "Latin_Name", "present", "cycle")], 
                           by = "Latin_Name", 
                           all.x = FALSE, all.y = TRUE)

# Summarize by species
RICH_inv_spplist3 <- RICH_inv_spplist2 %>% filter(!is.na(Year_added)) %>% 
                                           group_by(Latin_Name, cycle, Year_added) %>% 
...
# First join plant table to get common name
RICH_invlist2 <- merge(RICH_invlist_wide, plants[,c("Latin_Name", "Common")], 
                       by = "Latin_Name", all.x = TRUE, all.y = FALSE) %>% 
                 select(Latin_Name, Common, Year_added:Cycle4) #rearranging column order

# Sometimes multiple common names are listed in plants table, but we only want 1. Code below fixes that
RICH_invlist2$Common <- gsub(",.*$", "\\1", RICH_invlist2$Common) # The first two arguments are java
...
RICH_plot_nums <- RICH_plot_nums[ , names(RICH_invlist2)] #using names from RICH_invlist2 to order columns the same

RICH_invtbl <- rbind(RICH_invlist2, RICH_plot_nums)
