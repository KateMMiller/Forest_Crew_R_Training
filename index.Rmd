...
<details open><summary class='drop'>2. Tree diameter distribution figure <b style="color:red">Updated 7-30-20</b></summary>
...
<b style="color:red">Updated Code Chunk (see labs() line) 7-30-20</b>
```{r,c80, echo=T}
...
<details open><summary class='drop'>3. Regeneration by size class figure <b style="color:red">Updated 7-30-20</b></summary>
...
<b style="color:red">Updated Code Chunk 7-30-20</b>
```{r,c86, echo=T}
...
<details open><summary class='drop'>8. Average stocking index by cycle <b style="color:red">Updated 7-30-20</b></summary>
...
RICH_inv_spp2 <- RICH_inv_spp %>% group_by(Event_ID, Plot_Name, cycle) %>% 
...
length(unique(RICH_plot_ev$Plot_Name))

RICH_inv_merge <- merge(RICH_plot_ev[ , c("Event_ID", "Plot_Name", "Plot_Number", "cycle", "X_Coord", "Y_Coord")],
                        RICH_inv_spp2, by = c("Event_ID", "Plot_Name", "cycle"), all.x = TRUE, all.y = TRUE)
...
<details open><summary class='drop'>13. Invasive cover table</summary>
...
<details open><summary class='drop'>14. Invasive cover by species <b style="color:red">Updated 7-22-20</b></summary>
...
GETT_inv_pie <- joinQuadData(park = "GETT", from = 2016, to = 2019, speciesType = 'exotic')

GETT_inv_pie <- make_sppcode(GETT_inv_pie) #adding sppcode- function found in Metric 10.

top_spp <- data.frame(table(GETT_inv_pie$sppcode)) %>% arrange(-Freq)

# Just want to rename LON- to LONSPP so cleaner
GETT_inv_pie$sppcode[GETT_inv_pie$sppcode == "LON-"]<-"LONSPP"
sort(unique(GETT_inv_pie$sppcode))
```
There are 19 species on the list in GETT, requiring us to lump like with Metrics 11 & 12. </br>

<b style="color:red">Updated code chunk 7-22-20</b>
Note that the groups below include more species than occur in GETT to help with groupings for other parks. The exo_herb group, for example, should be used for all parks, and will be labeled as other exotic herbs in the map.
...
Now we need to take the sppcode and spread it wide and make into a shapefile.
...
GETT_inv_pwide <- GETT_inv_pie2 %>% select(Plot_Name, Plot_Number, X_Coord, Y_Coord, sppgrp, avg.cover) %>% 
                                    pivot_wider(names_from = sppgrp,
                                               ...
                                               values_fill = 0) #%>% 
                                    #select(-NOSPE) # Every plot has an invasive in GETT, 
                                    #so don't have NOSPE to remove recorded. 
...
```{r m15_1, results = 'hide', echo = TRUE}
...
```{r m15_3, echo = TRUE, results = 'hide'}
...
```
For the species that are exotic but not invasive, you'll want to put a * next to the latin name in the table, and add in the caption: "Species that are exotic to the mid-Atlantic region, but not considered invasive are indiated by *." The species that should get this star are Duchesnia indica, Glechoma hederacea, Persicaria longiseta, and Vinca minor. I'm going to do this in the code, but you can also do this by hand.
</br>

<b style="color:red">Updated code chunk 7-22-20</b>
```{r m_15_4, echo=TRUE, results = 'hide'}
