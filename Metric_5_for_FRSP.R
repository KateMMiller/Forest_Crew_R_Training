...
FRSP_regen_tbl2 <- FRSP_regen_tbl %>% group_by(Plot_Name, Plot_Number, cycle) %>% 
  ...
  select(Plot_Name, Plot_Number, cycle, seed, sap, stock)
...
FRSP_plots <- joinLocEvent(park = "FRSP", from = 2016, to = 2019, output='verbose') %>% 
              ...
              select(Plot_Name, Plot_Number, subunit) 
              

FRSP_regen_tbl4 <- merge(FRSP_plots, FRSP_regen_tbl3, by = c("Plot_Name", "Plot_Number"), 
                         all.x = TRUE, all.y = TRUE) %>% arrange(subunit, Plot_Name)
